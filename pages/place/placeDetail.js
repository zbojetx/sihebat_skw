...
export default class PlaceDetail extends Component {
	...
	handleBackButton = () => {
		this.props.from == 'isFromCategoryList' ? this.props.categoryId == null ? Actions.placeCategoryList({categoryId: this.props.categoryId}) : Actions.placeCategoryList({categoryId: this.props.categoryId, categoryName: this.props.categoryName}):
		Actions.placeList();
		return true;
	};
	...
	getPlaceDetail(){
		...
		if(this.state.userId != null) {
			axios({
				method: 'GET',
				url: Config.API_URL + '/api/v1/place/' + this.props.placeId + '?user=' + this.state.userId,
				data: {},
				timeout: 35000
			}).then((response) => {
				if (response.data.header.message == 'Success') {
					this.setState({
						detailPlace: response.data.data,
			...
			axios({
				method: 'GET',
				url: Config.API_URL + '/api/v1/place/' + this.props.placeId,
				data: {},
				timeout: 35000
			}).then((response) => {
				if (response.data.header.message == 'Success') {
	...
	putWasHere(status){
		...
		axios({
			method: 'PUT',
			url: Config.API_URL + '/api/v1/place/action',
			data: {
				mark: 'washere',
				status: status,
					...
					<View style={{ padding: 16 }}>
						<View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
							<View style={{ flex: 0.8, justifyContent: 'center' }}>
								<Text style={{ fontSize: 18, color: Colors.BLACK, fontFamily: Fonts.MEDIUM }}>{this.state.detailPlace.title}</Text>
							</View>
						...
						<View style={{ flexDirection: 'row', flex: 1, marginTop: 16 }}>
							<View style={{ flex: 0.7, justifyContent: 'center' }}>
								...
								<View style={{ flexDirection: 'row', marginTop: 8 }}>
									...
									<Text style={{ fontSize: 12, color: '#00b4db', fontFamily: Fonts.REGULAR, marginLeft: 4, textDecorationLine: 'underline' }}>{this.state.detailPlace.primary_contact}</Text>
								</View>
							</View>
						...
						<View style={{ marginTop: 24 }}>
							...
							<View>
								...
								<View style={{ alignItems: 'center', justifyContent: 'center', marginTop: 24 }}>
									<TouchableOpacity style={{ height: 40, backgroundColor: Colors.PRIMARY, borderRadius: 6, padding: 12 }} onPress={() => Actions.placeReview({ placeId: this.props.placeId, placeName: this.state.detailPlace.title })}>
