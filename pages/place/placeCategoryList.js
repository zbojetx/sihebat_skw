...
export default class PlaceCategoryList extends Component {
	...
	getPlaceList(){
		if(this.props.categoryId != null){
			axios({
				method: 'GET',
				url: Config.API_URL + '/api/v1/place?terms&paginate=1&limit=50&top=false&categories=[' + this.props.categoryId + ']',
				data: {},
				timeout: 35000
			}).then((response) => {
				if (response.data.header.message == 'Success') {
					this.setState({
						placeList: response.data.data.data
					})
				}
				else {
			...
			axios({
				method: 'GET',
				url: Config.API_URL + '/api/v1/place?terms&paginate=1&limit=50&top=true',
				data: {},
				timeout: 35000
			}).then((response) => {
				if (response.data.header.message == 'Success') {
	...
	placeList(item){
		return(
			<TouchableOpacity onPress={() => this.props.categoryId == null ? Actions.placeDetail({placeId: item.id, categoryId: null, from: 'isFromCategoryList'}) : Actions.placeDetail({placeId: item.id, categoryId: this.props.categoryId, categoryName: this.props.categoryName, from: 'isFromCategoryList'})}>
		...
		return(
			<Container>
				<Header style={{ backgroundColor: Colors.PRIMARY }}>
					...
					<Body>
						<Text style={{ fontSize: 18, color: Colors.WHITE, fontFamily: Fonts.REGULAR, marginLeft: 16 }}>{this.props.categoryId == null ? 'Tempat Populer' : this.props.categoryName}</Text>
					</Body>
					<Right/>
				</Header>
